...
import {
    ...
    ParsedEvent,
} from "@/packages/stacks/contracts";
import { upsertWallets } from "@/packages/db/actions";
import { getAddressesStSTXBalance } from "@/packages/stacks/accounts";
import { contracts } from "@/packages/stacks/constants";

// export async function track(event: SNSEvent, _: Context): Promise<void> {
export async function track(event: SNSEvent): Promise<void> {
...
async function processBlockEvents(block: Block): Promise<string[]> {
    ...
    const rawEvents = results
        .flat()
        .filter(
            (e) => e.event_type === "smart_contract_log",
        ...
        switch (event.contract_id) {
            ...
            case contracts.core:
                if (event.action.action?.value === "deposit")
                    addresses.push(
                        event.action.data?.value.stacker?.value ?? ".",
                    );
                break;
            // Transfer stSTX
            case contracts.token:
                if (event.action.action?.value === "transfer")
                    addresses.push(
                        event.action.data?.value.recipient?.value ?? ".",
                    );
                break;
            // Arkadiko migration
    ...
    const addresses = txs
        ...
        .filter(
            (tx) =>
                ...
                [contracts.swap1, contracts.swap2].includes(

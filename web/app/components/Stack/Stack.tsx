        ...
        <div className="mt-6 w-full">
          ...
          <div className="grid gap-x-2 grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 xl:grid-cols-1">
            <StackingPartner
              ...
              referral={referralAddress ? `${referralAddress.slice(0, 4)}...${referralAddress.slice(-4)}` : undefined}
            />
            {/*<StackingPartner
          ...
          <div className="relative mt-4 flex w-full flex-col items-center justify-center gap-4 rounded-lg bg-sd-gray-light p-6 text-center font-medium">
              ...
              suffixIcon={
                <>
                  ...
                  <Tooltip anchorSelect="#points-boost" place="top">
                    Any amount you stack with us in the next two weeks - before Cycle 84 - will have a 20x points
                    multiplier applied!
                  </Tooltip>
                </>
              }
            ...
            <div className="flex flex-row justify-center gap-1">
              ...
              {Number.isNaN(amount.stx) ? (
                ...
                <span className="flex flex-row gap-1">
                  ~
                  {(stackingPartner === "stackingdao" ? amount.ststx : bitflow.ststx).toLocaleString("en-US", {
                    minimumFractionDigits: 2,
            ...
            <div className="flex flex-row justify-center gap-1">
              ...
              {Number.isNaN(amount.stx) ? (
                ...
                <span className="flex flex-row gap-1">
                  ~
                  {((stackingPartner === "stackingdao" ? amount.ststx : bitflow.ststx) * 20).toLocaleString("en-US", {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}
                  <StSTXIcon width={20} height={20} />
          ...
          {buttonState === "insufficient" && (
            <div className="flex items-center justify-center mt-4 mb-2 py-2 px-4 w-full rounded-lg bg-red-400 text-left md:text-center font-medium text-sm text-white">
              <span>⚠️</span>
              <p className="ml-2">
                Please make sure to leave enough STX (currently {currency.long.format(gasFeeTolerance)} STX) for
                transaction gas
              </p>
            </div>
          )}

          <LinkButton type={stackingPartner === "bitflow" ? "link" : "button"} href="https://app.bitflow.finance/trade">
            <button
              type="button"
...
function StackingPartner({ name, logo, selected, onClick, recommended, ratio, referral }: StackingPartnerProps) {
  return (
    <div
      onClick={onClick}
      className={cn(
    ...
    >
      ...
      {!!referral && (
        <div className="flex flex-row justify-between">
          ...
          <p className="flex flex-row text-right text-sm font-semibold gap-1 items-center">{referral}</p>
        </div>
      )}
    </div>
  );

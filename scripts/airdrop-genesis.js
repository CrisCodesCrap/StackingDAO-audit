...
const cvs2 = pieces[1].map((piece) => tx.tupleCV({ 'recipient': tx.standardPrincipalCV(piece), 'type': tx.uintCV(1) }));
dropIt(cvs2, 63);

const cvs3 = pieces[2].map((piece) => tx.tupleCV({ 'recipient': tx.standardPrincipalCV(piece), 'type': tx.uintCV(1) }));
dropIt(cvs3, 64);

const cvs4 = pieces[3].map((piece) => tx.tupleCV({ 'recipient': tx.standardPrincipalCV(piece), 'type': tx.uintCV(1) }));
dropIt(cvs4, 65);

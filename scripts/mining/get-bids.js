...
async function exec() {
  ...
  while (offset < 120) {
    const url = `https://api.mainnet.hiro.so/extended/v2/blocks?limit=30&offset=${offset}`;
    const response = await fetch(url, { credentials: 'omit' });
    const data = await response.json();

    await asyncForEach(data['results'], async (result) => {
      const blockUrl = `https://api.mainnet.hiro.so/extended/v2/blocks/${result.height}/transactions`;

...
class DirectHelpers {
  ...
  subtractDirectStacking(caller: Account, user: string, amount: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("direct-helpers-v1", "subtract-direct-stacking", [
        types.principal(user),
        types.uint(amount * 1000000),
      ], caller.address)
    ]);
  ...
  stopDirectStacking(caller: Account, user: string) {
    let block = this.chain.mineBlock([
      Tx.contractCall("direct-helpers-v1", "stop-direct-stacking", [
        types.principal(user),
      ], caller.address)
    ]);
    return block.receipts[0].result;
  ...
  subtractDirectStackingUser(caller: Account, amount: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("direct-helpers-v1", "subtract-direct-stacking-user", [
        types.uint(amount * 1000000),
      ], caller.address)
    ]);
    return block.receipts[0].result;
  ...
  stopDirectStackingUser(caller: Account) {
    let block = this.chain.mineBlock([
      Tx.contractCall("direct-helpers-v1", "stop-direct-stacking-user", [
      ], caller.address)
    ]);
    return block.receipts[0].result;
  }

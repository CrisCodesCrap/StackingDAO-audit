...
class DelegatesHandler {
  ...
  getLastSelectedPool(delegate: string) {
    return this.chain.callReadOnlyFn("delegates-handler-v1", "get-last-selected-pool", [
      types.principal(delegate)
    ], this.deployer.address);
  }

  getTargetLockedAmount(delegate: string) {
    return this.chain.callReadOnlyFn("delegates-handler-v1", "get-target-locked-amount", [
      types.principal(delegate)
    ], this.deployer.address);
  }

  getLastLockedAmount(delegate: string) {
    return this.chain.callReadOnlyFn("delegates-handler-v1", "get-last-locked-amount", [
      types.principal(delegate)
    ], this.deployer.address);
  }
  ...
  getStxAccount(user: string) {
    return this.chain.callReadOnlyFn("delegates-handler-v1", "get-stx-account", [
      types.principal(user)
    ], this.deployer.address);
  }

  calculateRewards(delegate: string) {
    return this.chain.callReadOnlyFn("delegates-handler-v1", "calculate-rewards", [
      types.principal(delegate)
    ], this.deployer.address);
  }

  calculateExcess(delegate: string) {
    return this.chain.callReadOnlyFn("delegates-handler-v1", "calculate-excess", [
      types.principal(delegate)
    ], this.deployer.address);
  }
  ...
  revoke(caller: Account, delegate: string) {
    let block = this.chain.mineBlock([
      Tx.contractCall("delegates-handler-v1", "revoke", [
        types.principal(delegate),
        types.principal(qualifiedName("reserve-v1")),
        types.principal(qualifiedName("rewards-v1")),
      ], caller.address)
  ...
  revokeAndDelegate(caller: Account, delegate: string, amount: number, delegateTo: string, untilBurnHeight: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("delegates-handler-v1", "revoke-and-delegate", [
        types.principal(delegate),
        types.principal(qualifiedName("reserve-v1")),
        types.principal(qualifiedName("rewards-v1")),

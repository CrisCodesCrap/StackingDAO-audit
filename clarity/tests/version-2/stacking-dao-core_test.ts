...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let result = await stStxToken.mintForProtocol(deployer, 100, qualifiedName("stacking-dao-core-v1"));
    result.expectOk().expectBool(true);

    let call = await stStxToken.getBalance(qualifiedName("stacking-dao-core-v1"))
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let block = chain.mineBlock([
      Tx.contractCall("stacking-dao-core-v2", "init-withdraw", [
        types.principal(qualifiedName("fake-reserve")),
        types.principal(qualifiedName("direct-helpers-v1")),
        types.uint(100 * 1000000),
      ], deployer.address)
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let block = chain.mineBlock([
      Tx.contractCall("stacking-dao-core-v2", "cancel-withdraw", [
        types.principal(qualifiedName("fake-reserve")),
        types.principal(qualifiedName("direct-helpers-v1")),
        types.uint(0),
        types.none()
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let result = dataPools.setPoolCommission(deployer, qualifiedName("stacking-pool-v1"), 0);
    result.expectOk().expectBool(true);
    ...
    // Add rewards
    result = await rewards.addRewards(wallet_2, qualifiedName("stacking-pool-v1"), 10000);
    result.expectOk().expectBool(true);
    ...
    // Add rewards
    result = await rewards.addRewards(wallet_2, qualifiedName("stacking-pool-v1"), 18000);
    result.expectOk().expectBool(true);
    result = await rewards.processRewards(wallet_2);
    result.expectOk().expectBool(true);

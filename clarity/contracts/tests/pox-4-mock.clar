...
(define-constant PREPARE_CYCLE_LENGTH (if is-in-mainnet u100 u3))

;; Default length of the PoX reward cycle, in burnchain blocks.
(define-constant REWARD_CYCLE_LENGTH (if is-in-mainnet u2100 u21))
...
;;    (6) (reward-cycle-to-burn-height (+ lock-period first-reward-cycle)) == (get unlock-height (stx-account-mock stacker))
...
(define-public (delegate-stack-stx (stacker principal)
    ...
    (let ((first-reward-cycle (+ u1 (current-pox-reward-cycle)))
      ...
      ;; MOCK - Update stx-account-map
      (map-set stx-account-map { account: stacker } (merge (stx-account-mock stacker) { 
          locked: amount-ustx, 

...
;; Get pair data
(define-read-only (get-pair-data (x-token <og-sip-010-trait>) (y-token <og-sip-010-trait>) (lp-token <og-sip-010-trait>)) 
...
;; Get DX
(define-read-only (get-dx (y-token <og-sip-010-trait>) (x-token <og-sip-010-trait>) (lp-token <lp-trait>) (y-amount uint))
    (let 
        (
...
;; Get DY
(define-read-only (get-dy (x-token <og-sip-010-trait>) (y-token <og-sip-010-trait>) (lp-token <lp-trait>) (x-amount uint))
    (let 
        (
...
(define-public (swap-x-for-y (x-token <og-sip-010-trait>) (y-token <og-sip-010-trait>) (lp-token <lp-trait>) (x-amount uint) (min-y-amount uint)) 
    (let 
        (
            (swapper tx-sender)
            (pair-data (unwrap! (map-get? PairsDataMap {x-token: (contract-of x-token), y-token: (contract-of y-token), lp-token: (contract-of lp-token)}) (err "err-no-pair-data")))
...
(define-public (swap-y-for-x (y-token <og-sip-010-trait>) (x-token <og-sip-010-trait>) (lp-token <lp-trait>) (y-amount uint) (min-x-amount uint)) 
    (let 
        (
            (swapper tx-sender)
            (pair-data (unwrap! (map-get? PairsDataMap {x-token: (contract-of x-token), y-token: (contract-of y-token), lp-token: (contract-of lp-token)}) (err "err-no-pair-data")))
...
(define-public (add-liquidity (x-token <og-sip-010-trait>) (y-token <og-sip-010-trait>) (lp-token <lp-trait>) (x-amount-added uint) (y-amount-added uint) (min-lp-amount uint) )
    (let 
        (
...
(define-public (withdraw-liquidity (x-token <og-sip-010-trait>) (y-token <og-sip-010-trait>) (lp-token <lp-trait>) (lp-amount uint) (min-x-amount uint) (min-y-amount uint))
    (let 
        (
            ;; Grabbing all data from PairsDataMap
            (current-pair (unwrap! (map-get? PairsDataMap {x-token: (contract-of x-token), y-token: (contract-of y-token), lp-token: (contract-of lp-token)}) (err "err-no-pair-data")))
...
(define-public (create-pair (x-token <og-sip-010-trait>) (y-token <og-sip-010-trait>) (lp-token <lp-trait>) (amplification-coefficient uint) (pair-name (string-ascii 32)) (initial-x-bal uint) (initial-y-bal uint))
    (let 
        (
            (lp-owner tx-sender)
...
(define-public (set-pair-approval (x-token <og-sip-010-trait>) (y-token <og-sip-010-trait>) (lp-token <lp-trait>) (approval bool))
    (let 
        (
            (current-pair (unwrap! (map-get? PairsDataMap {x-token: (contract-of x-token), y-token: (contract-of y-token), lp-token: (contract-of lp-token)}) (err "err-no-pair-data")))
        )
...
(define-public (change-amplification-coefficient (x-token <og-sip-010-trait>) (y-token <og-sip-010-trait>) (lp-token <lp-trait>) (amplification-coefficient uint))
    (let 
        (
            (current-pair (unwrap! (map-get? PairsDataMap {x-token: (contract-of x-token), y-token: (contract-of y-token), lp-token: (contract-of lp-token)}) (err "err-no-pair-data")))
            (current-admins (var-get admins))
